...
const botBehaviorStates = new Map<string, BehaviorTable>();

function checkactions(bot: test.SimulatedPlayer): void {
...
function initializeBotBehavior(botId: string): void {
...
function tntActions(bot: test.SimulatedPlayer): void {
...
function normalActions(bot: test.SimulatedPlayer): void {
...
function idleActions(bot: test.SimulatedPlayer): void {
...
function waterActions(bot: test.SimulatedPlayer): void {
...
function agresiveJump(bot: test.SimulatedPlayer): void {
...
function getDistance(a: mc.Vector3, b: mc.Vector3): number {
    const dx = a.x - b.x;
    const dy = a.y - b.y;
    const dz = a.z - b.z;
...
function startAttack(bot: test.SimulatedPlayer, tag: string): void {
    ...
    let closest: mc.Player | undefined;
    ...
    if (Math.random() < 3.35) {
        const items: { item: string, condition?: () => boolean, extra?: () => void; }[] = [
...
function useElytra(bot: test.SimulatedPlayer): void {
...
function stopAll(bot: test.SimulatedPlayer): void {
    const dataTimer = timers.get(bot.id);
