...
const listOfBots: gametest.SimulatedPlayer[] = [];
...
function generateFakePly(test: gametest.Test, total: number = 5): void {
...
function startBots(bots: gametest.SimulatedPlayer[]): void {
...
function spectBehavior(bot: gametest.SimulatedPlayer): void {
    stopAll(bot);
...
function tntBehavior(bot: gametest.SimulatedPlayer): void {
    stopAll(bot);
...
function normalBehavior(bot: gametest.SimulatedPlayer): void {
    stopAll(bot);
...
function waterScape(bot: gametest.SimulatedPlayer): void {
    ...
    if (Math.random() > 0.6) {
        ...
        const validTargets = nearbyPlayers.filter(ply => {
            if (bot.hasTag('tntPly') && ply.hasTag('tntPly')) return false;
            if (ply.hasTag('spectMode')) return false;
            return true;
        });
...
function netBehavior(bot: gametest.SimulatedPlayer): void {
    stopAll(bot);
...
function stopAll(bot: gametest.SimulatedPlayer): void {
...
function equipItems(bot: gametest.SimulatedPlayer): void {
    const itemsToGive: { tag: string, item: string, condition?: boolean, secondaryItem?: string; }[] = [
...
function checkBlock(bot: gametest.SimulatedPlayer): void {
...
function getNearbyPlayers(bot: gametest.SimulatedPlayer, radius: number = 7): mc.Entity[] {
...
function getNearPlayers(bot: gametest.SimulatedPlayer, filters: mc.EntityQueryOptions = {}): mc.Entity[] {
    return bot.dimension.getEntities(filters);
}

function calculateDistance(loc1: mc.Vector3, loc2: mc.Vector3): number {
