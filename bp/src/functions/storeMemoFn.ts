...
interface ListOfShopItems {
    [key: number]: {
        tag: string;
        selection: string;
        count: number;
        isGolden: boolean;
        item: mc.ItemStack | string;
    };
}
 ...
 */
export function showStore(ply: mc.Player): void {
    const allTags = ply.getTags();
    const shopItems = getDataItems(0, true) as ListOfShopItems;
 ...
 */
function shopManage(ply: mc.Player, selection: number, playerPoints: number): void {
    const item = getDataItems(selection) as ShopItem;
    const allTags = ply.getTags();
    const objCoins = mc.world.scoreboard.getObjective('totalPoints');
 ...
 */
function getDynamicPrice(basePrice: number, playerPoints: number): number {
 ...
 */
function getDataItems(selection: number = 0, getAllVar: boolean = false): ShopItem | ListOfShopItems {
    const shopItems: ListOfShopItems = {
 ...
 * @returns {void}
 */
export function getItemsFromShop(ply: mc.Player): void {
    const allTags = ply.getTags();
    const inv = ply.getComponent(mc.EntityComponentTypes.Inventory)?.container;
    const allData = getDataItems(0, true) as ListOfShopItems;
