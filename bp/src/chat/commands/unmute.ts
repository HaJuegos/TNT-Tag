...
/**
 * Variable con los eventos del comando !unmuteCmd
 * @type {CommandBase}
 */
const unmuteCmd: CommandBase = {
    name: "unmute",
    alias: ["rmute"],
    descripcion: { translate: "chat.descripcion_cmd.unmuted" } as mc.RawText,
    usage: {
        translate: "chat.help.usage", with: {
            rawtext: [
                { translate: "chat.help.command_ej.unmute" },
    ...
    execute: (ctxt) => {
        ...
        if (!targetPlyArg) {
            ...
            ply.sendMessage({ translate: "chat.error.nomuted", with: { rawtext: [{ text: targetPlyArg }] } });
            ply.playSound("ui.powerup.in_cooldown");
            return;
        }
        ...
        if (targetPly) {
            targetPly.removeTag('muted');
            ply.sendMessage({ translate: "chat.success.unmutedPly", with: { rawtext: [{ text: targetPly.name }] } });
