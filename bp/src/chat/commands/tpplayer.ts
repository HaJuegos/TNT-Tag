...
/**
 * Variable con los eventos del comando tpplayer
 * @type {CommandBase}
 */
const tpPlayerCmd: CommandBase = {
    name: "tpplayer",
    alias: ["tpp"],
    descripcion: { translate: "chat.descripcion_cmd.tpplayer" } as mc.RawText,
    usage: {
        translate: "chat.help.usage", with: {
            rawtext: [
                { translate: "chat.help.command_ej.tpp" },
    ...
    execute: (ctxt) => {
        ...
        if (!targetPlyArg && !targetPly) {
            ply.sendMessage({ translate: "chat.error.noplysnormal" });
            ...
            ply.playSound("ui.powerup.in_cooldown");
            return;
        } else if (targetPlyArg && targetPly && !targetPly.hasTag('normalPly')) {
            ...
            ply.sendMessage({ translate: "chat.error.hastnt", with: { rawtext: [{ text: targetPlyArg }] } });
            ply.playSound("ui.powerup.in_cooldown");
            return;
        }
 ...
 */
function getPlys(namePly?: string): mc.Player | undefined {
    const plys = mc.world.getAllPlayers().filter(ply => ply.name == namePly);
    const plysTNT = mc.world.getAllPlayers().filter(ply => ply.hasTag('normalPly'));
