 ...
 */
const calculatorEvent: CustomComponentBase = {
 ...
 * @returns {void}
 */
function calculatorVotes(ply: mc.Player): void {
    ...
    maps.forEach(map => {
        votations.button(
            { translate: map.id === 'random' ? 'ui.button_random' : `ui.button_vote${map.id}` },
            `textures/ui/custom/map_icons/${map.id === 'random' ? 'random' : 'map' + map.id}`
        );
    });
 ...
 */
async function showVoteWarning(ply: mc.Player, mapID: string): Promise<boolean> {
 ...
 */
function registerVote(ply: mc.Player, mapSelected: any): void {
