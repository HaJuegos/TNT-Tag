 ...
 * @returns {void}
 */
function checkEffectsMaps(entity: mc.Entity): void {
    ...
    for (const ply of mc.world.getAllPlayers().filter(p => !p.hasTag('spectMode'))) {
        ...
        const hasEffect = (effectId: string, amplifier = 1) =>
            effects.some(e => e.typeId == effectId && e.amplifier >= amplifier);
 ...
 * @returns {void}
 */
function checkInVoid(entities: mc.Player[] | mc.Entity[], entity: mc.Entity): void {
    const objMap = mc.world.scoreboard.getObjective('mapSelected');
    const mapSelected = objMap?.getScore(entity) ?? 0;
 ...
 * @returns {void}
 */
function checkInBlock(ply: mc.Player, entity: mc.Entity): void {
    const objMap = mc.world.scoreboard.getObjective('mapSelected');
    const mapSelected = objMap?.getScore(entity) ?? 0;
    const randomTp = getRandomSpawnMap(mapSelected);
 ...
 */
function hasItemInInv(inv: mc.Container, itemID: string): boolean {
 ...
 * @returns {Void}
 */
function startTimer(timer: TimerLoopBase): void {
