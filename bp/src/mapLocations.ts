...
const usedSpawns: Map<number, Set<number>> = new Map();

/**
 * Lista de spawn de todos los mapas disponibles.
 * @type {MapData[]}
 */
const mapsLocations: MapData[] = [
    {
        id: 1,
        spawnZones: [
            { x: 15, y: -60, z: -116 },
...
/**
 * Obtiene de forma aleatoria las zonas de spawn del mapa seleccionado.
 * @param {number} mapId ID del Mapa elejido.
 * @returns {Vector3 | undefined} Devuelve {@link Vector3} aleatorio del mapa, si sale un error, entonces undefined. 
 */
function getRandomSpawnMap(mapId: number): Vector3 | undefined {
    const map = mapsLocations.find(map => map.id == mapId);

    if (!map) return;

    let usedI = usedSpawns.get(mapId);

    if (!usedI) {
        usedI = new Set();
        usedSpawns.set(mapId, usedI);
    }

    if (usedI.size >= map.spawnZones.length) {
        usedI.clear();
    }

    const availableI = map.spawnZones.map((_, i) => i).filter(i => !usedI.has(i));
    const randomI = availableI[Math.floor(Math.random() * availableI.length)];

    usedI.add(randomI);

    return map.spawnZones[randomI];
}

export { getRandomSpawnMap };
